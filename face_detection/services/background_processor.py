...
async def fetch_frames_task(camera_id: str, queue: asyncio.Queue):
    """
    Fetches frames from the Redis face_group stream for a given camera.
    Input stream: stream:{camera_id}:face_group
    """
    input_stream = f"stream:{camera_id}:{GROUP_SUFFIX}"
    await redis_manager.ensure_group(input_stream, GROUP_SUFFIX)
    print(f"[face/{camera_id}] 🟢 Fetch task started (consumer: {CONSUMER_ID})")
    ...
    while True:
        try:
            ...
            for msg_id_bytes, fields in entries:
                ...
                if frame is not None:
                    ...
                    if frames_received % LOG_EVERY_N_FRAMES == 0:
                        print(f"[face/{camera_id}] 📥 Pulled {frames_received} frames from Redis (latest frame_id={frame_id})")
            ...
            print(f"[face/{camera_id}] 🛑 Fetch task stopped (total frames received: {frames_received}).")
            break
        except Exception as e:
            print(f"[face/{camera_id}] ❌ Fetch Error: {e}")
            await asyncio.sleep(1)


async def process_camera_task(camera_id: str, queue: asyncio.Queue):
    """
    Analyzes frames with FaceAnalyzer and pushes ONLY metadata (detections)
    to the face result stream: face:{camera_id}
    """
    print(f"[face/{camera_id}] 🟢 Analysis task started.")
    ...
    while True:
        try:
            ...
            if frame_id:
                ...
                if detections:
                    if has_recognition:
                        ...
                        print(f"[face/{camera_id}] 🔍 RECOGNIZED: {names} | frame={frame_id}")
                    else:
                        print(f"[face/{camera_id}] 👤 Face(s) detected: {det_count} unknown | frame={frame_id}")
                    clean_frames = 0
                else:
                    clean_frames += 1
                    if clean_frames % LOG_EVERY_N_FRAMES == 0:
                        print(f"[face/{camera_id}] ✅ Processed {frames_processed} frames | last {clean_frames} with no faces")

                payload = {
                    "frame_id": frame_id,
                    "detections": json.dumps(detections),
                    "detections_count": str(det_count),
                    "has_recognition": str(has_recognition),
            ...
            print(f"[face/{camera_id}] 🛑 Analysis task stopped (total processed: {frames_processed}).")
            break
        except Exception as e:
            print(f"[face/{camera_id}] ❌ Analysis Error: {e}")
            await asyncio.sleep(1)
...
async def start_cameras(camera_ids: List[str]):
    for cam in camera_ids:
        if cam in active_monitors:
            print(f"[face] Camera {cam} already active. Skipping.")
            continue
